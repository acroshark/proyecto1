const quizQuestions = [
  {
    question: 'What breed of dog was Marley in the film "Marley & Me" (2008)?',
    answers: [
      "Labrador Retriever",
      "Dalmatian",
      "Golden Retriever",
      "Shiba Inu",
    ],
    correct: "Labrador Retriever",
  },
  // ...Aqui van las demás preguntas del quiz

  {
    question: 'Which of the following is not the name of a "Bond Girl"? ',
    answers: ["Pam Bouvier", "Vanessa Kensington", "Wai Lin", "Mary Goodnight"],
    correct: "Vanessa Kensington",
  },
  {
    question:
      "Which movie released in 2016 features Superman and Batman fighting?",
    answers: [
      "Batman v Superman: Black of Knight",
      "Batman v Superman: Knightfall",
      "Batman v Superman: Superapocalypse",
      "Batman v Superman: Dawn of Justice",
    ],
    correct: "Batman v Superman: Dawn of Justice",
  },
  {
    question:
      "Daniel Radcliffe became a global star in the film industry due to his performance in which film franchise?",
    answers: ["Ted", "Spy Kids", "Harry Potter", "Pirates of the Caribbean "],
    correct: "Harry Potter",
  },
  {
    question:
      'Who starred as Bruce Wayne and Batman in Tim Burton\'s 1989 movie "Batman"?',
    answers: ["Michael Keaton", "Val Kilmer", "Adam West", "George Clooney"],
    correct: "Michael Keaton",
  },
  {
    question:
      "What name did Tom Hanks give to his volleyball companion in the film `Cast Away`?",
    answers: ["Billy", "Jones", "Wilson", "Friday"],
    correct: "Wilson",
  },
  {
    question: 'Who directed "E.T. the Extra-Terrestrial" (1982)?',
    answers: [
      "Steven Spielberg",
      "Stanley Kubrick",
      "James Cameron",
      "Tim Burton",
    ],
    correct: "Steven Spielberg",
  },
  {
    question:
      "Which animated movie was first to feature a celebrity as a voice actor?",
    answers: [
      "Toy Story",
      "James and the Giant Peach",
      "Aladdin",
      "The Hunchback of Notre Dame",
    ],
    correct: "Aladdin",
  },
  {
    question: "When was the movie 'Con Air' released?",
    answers: ["1997", "1985", "1990", "1999"],
    correct: "1997",
  },
  {
    question: "Who wrote and directed the 1986 film 'Platoon'?",
    answers: [
      "Francis Ford Coppola",
      "Oliver Stone",
      "Stanley Kubrick",
      "Michael Cimino",
    ],
    correct: "Oliver Stone",
  },
  {
    question:
      'Who played Deputy Marshal Samuel Gerard in the 1993 film "The Fugitive"?',
    answers: [
      "Harrison Ford",
      "Tommy Lee Jones",
      "Martin Landau",
      "Harvey Keitel",
    ],
    correct: "Tommy Lee Jones",
  },
  {
    question: 'Who played the female lead in the 1933 film "King Kong"?',
    answers: ["Fay Wray", "Mae West", "Jean Harlow", "Vivien Leigh"],
    correct: "Fay Wray",
  },
  {
    question:
      "Which actress danced the twist with John Travolta in 'Pulp Fiction'?",
    answers: ["Kathy Griffin", "Pam Grier", "Bridget Fonda", "Uma Thurman"],
    correct: "Uma Thurman",
  },
  {
    question: 'Who starred in the film 1973 movie "Enter The Dragon"?',
    answers: [" Yun-Fat Chow", "Jackie Chan", "Jet Li", "Bruce Lee"],
    correct: "Bruce Lee",
  },
  {
    question:
      'Who directed the movies "Pulp Fiction", "Reservoir Dogs" and "Django Unchained"?',
    answers: [
      "Quentin Tarantino",
      "Steven Spielberg",
      "James Cameron",
      "Martin Scorcese",
    ],
    correct: "Quentin Tarantino",
  },
  {
    question:
      "The Queen song `A Kind Of Magic` is featured in which 1986 film?",
    answers: ["Flash Gordon", "Labyrinth", "Howard the Duck", "Highlander"],
    correct: "Highlander",
  },
  {
    question: "What was Dorothy's surname in 'The Wizard Of Oz'?",
    answers: ["Parker", "Day", "Perkins", "Gale"],
    correct: "Gale",
  },
  {
    question:
      'This movie contains the quote, "I feel the need ... the need for speed!"',
    answers: ["Top Gun", "Days of Thunder", "The Color of Money", "Cocktail"],
    correct: "Top Gun",
  },
  {
    question: 'This movie contains the quote, "Nobody puts Baby in a corner."',
    answers: [
      "Pretty in Pink",
      "Ferris Bueller's Day Off",
      "Dirty Dancing",
      "Three Men and a Baby",
    ],
    correct: "Dirty Dancing",
  },
  {
    question:
      'What is the orange and white bot\'s name in "Star Wars: The Force Awakens"?',
    answers: ["AA-A", "BB-3", "BB-8", "R2-D2"],
    correct: "BB-8",
  },
  {
    question: "What was the first James Bond film?",
    answers: ["Goldfinger", "Dr. No", "From Russia With Love", "Thunderball"],
    correct: "Dr. No",
  },
  {
    question: "Which of these movies did Jeff Bridges not star in?",
    answers: ["The Giver", "Tron: Legacy", "True Grit", "The Hateful Eight"],
    correct: "The Hateful Eight",
  },
  {
    question:
      "Which movie includes a giant bunny-like spirit who has magic powers including growing trees?",
    answers: [
      "Hop",
      "My Neighbor Totoro",
      "Alice in Wonderland",
      "Rise of the Guardians",
    ],
    correct: "My Neighbor Totoro",
  },
  {
    question: "Who plays Alice in the Resident Evil movies?",
    answers: ["Madison Derpe", "Kim Demp", "Milla Jovovich", "Milla Johnson"],
    correct: "Milla Jovovich",
  },
  {
    question: "What is the oldest Disney film?",
    answers: [
      "Dumbo",
      "Snow White and the Seven Dwarfs",
      "Pinocchio",
      "Fantasia",
    ],
    correct: "Snow White and the Seven Dwarfs",
  },
  {
    question:
      "Who is the main protagonist in, the 1985 film, Back to the Future?",
    answers: [
      "Marty McFly",
      "George McFly",
      'Emmett "Doc" Brown',
      "Biff Tannen",
    ],
    correct: "Marty McFly",
  },
  {
    question:
      'In the movie "Cast Away" the main protagonist\'s best friend while on the island is named',
    answers: ["Willy", "Wilson", "Carson", "Jackson"],
    correct: "Wilson",
  },
  {
    question:
      'In "Jurassic World", what is the name of the dinosaur that is a genetic hybrid?',
    answers: [
      "Mosasaurus",
      "Pteranodon",
      "Indominus Rex",
      "Tyrannosaurus Rex ",
    ],
    correct: "Indominus Rex",
  },
  {
    question: "Which actor plays Obi-Wan Kenobi in Star Wars Episodes I-lll?",
    answers: [
      "Alec Guinness",
      "Ewan McGregor",
      "Liam Neeson",
      "Hayden Christensen",
    ],
    correct: "Ewan McGregor",
  },
  {
    question:
      'In the movie "Spaceballs", what are the Spaceballs attempting to steal from Planet Druidia?',
    answers: ["The Schwartz", "Air", "Meatballs", "Princess Lonestar"],
    correct: "Air",
  },
  {
    question:
      "In the movie Gremlins, after what time of day should you not feed Mogwai?",
    answers: ["Midnight", "Morning", "Evening", "Afternoon"],
    correct: "Midnight",
  },
  {
    question:
      'When does "Rogue One: A Star Wars Story" take place chronologically in the series?',
    answers: [
      "Before Episode 1",
      "Between Episode 4 and 5",
      "After Episode 6",
      "Between Episode 3 and 4",
    ],
    correct: "Between Episode 3 and 4",
  },
  {
    question: 'In the 2012 film, "The Lorax", who is the antagonist?',
    answers: ["Grammy Norma", "The Once-Ler", "Ted Wiggins", "Aloysius O'Hare"],
    correct: "Aloysius O'Hare",
  },
  {
    question:
      'At the end of the 2001 film "Rat Race", whose concert do the contestants crash?',
    answers: ["Sum 41", "Smash Mouth", "Linkin Park", "Bowling for Soup"],
    correct: "Smash Mouth",
  },
  {
    question: "Who plays the character of Po in the Kung Fu Panda movies?",
    answers: [
      "McConahey Ramses",
      "Jack Black",
      "Mirana Jonnes",
      "Jim Petersson",
    ],
    correct: "Jack Black",
  },
  {
    question:
      "The 2016 Disney animated film 'Moana' is based on which culture?",
    answers: ["Japanese", "Nordic", "Polynesian", "Native American"],
    correct: "Polynesian",
  },
  {
    question: 'In the 1995 film "Balto", who are Steele\'s accomplices?',
    answers: [
      "Dusty, Kirby, and Ralph",
      "Jenna, Sylvie, and Dixie",
      "Kaltag, Nikki, and Star",
      "Nuk, Yak, and Sumac",
    ],
    correct: "Kaltag, Nikki, and Star",
  },
  {
    question:
      'What is the name of the island that "Jurassic Park" is built on?',
    answers: ["Isla Pena", "Isla Sorna", "Isla Muerta", "Isla Nublar"],
    correct: "Isla Nublar",
  },
  {
    question:
      "What was Bruce Campbell's iconic one-liner after getting a chainsaw hand in Evil Dead 2?",
    answers: ["Perfect.", "Groovy.", "Gnarly.", "Nice."],
    correct: "Groovy.",
  },
  {
    question:
      'For the film "Raiders of The Lost Ark", what was Harrison Ford sick with during the filming of the Cairo chase?',
    answers: ["Anemia", "Constipation", "Acid Reflux ", "Dysentery"],
    correct: "Dysentery",
  },
  {
    question: "What was the first feature-length computer-animated movie?",
    answers: ["Tron", "101 Dalmatians", "Lion king", "Toy Story"],
    correct: "Toy Story",
  },
  {
    question: "In Big Hero 6, what fictional city is the Big Hero 6 from?",
    answers: ["Tokysisco", "Sankyo", "San Fransokyo", "San Tokyo"],
    correct: "San Fransokyo",
  },
  {
    question: "Where does the original Friday The 13th movie take place?",
    answers: [
      "Packanack",
      "Higgins Haven",
      "Camp Crystal Lake",
      "Camp Forest Green",
    ],
    correct: "Camp Crystal Lake",
  },
  {
    question:
      'In which film does Humphrey Bogart say the famous line, "Here\'s looking at you, kid"?',
    answers: [
      "The Maltese Falcon",
      "Citizen Kane",
      "The Treasure of the Sierra Madre",
      "Casablanca",
    ],
    correct: "Casablanca",
  },
  {
    question:
      "What is the name of the fictional retro-mod band starring Austin Powers as the lead vocalist?",
    answers: ["Ming Tea", "Spear Mint", "Mister E", "Cough Fi"],
    correct: "Ming Tea",
  },
  {
    question: "Who directed the Kill Bill movies?",
    answers: [
      "Stanley Kubrick",
      "Arnold Schwarzenegger",
      "Quentin Tarantino",
      "David Lean",
    ],
    correct: "Quentin Tarantino",
  },
  {
    question: 'In "ALF", what was ALF\'s real name?',
    answers: [
      "Gordon Foster",
      "Gordon von Gam",
      "Gordon Milipp",
      "Gordon Shumway",
    ],
    correct: "Gordon Shumway",
  },
  {
    question:
      "Which of the following actors has only been in a Quentin Tarantino directed film once?",
    answers: [
      "Christoph Waltz",
      "John Travolta",
      "Harvey Keitel",
      "Samuel L. Jackson",
    ],
    correct: "John Travolta",
  },
  {
    question:
      "What historical time period was the center of the Assassin’s Creed movie (2016)?",
    answers: [
      "Victorian England",
      "Spanish Inquisition",
      "Colonial America",
      "French Revolution",
    ],
    correct: "Spanish Inquisition",
  },
  {
    question:
      'In the movie "V for Vendetta," what is the date that masked vigilante "V" urges people to remember?',
    answers: ["November 4th", "September 5th", "November 6th", "November 5th"],
    correct: "November 5th",
  },
];

/*  es un espacio deliberado
as
sdafsd
asdf
asdf
asdf
asdfasdfasdf */

let currentQuestion = 0;
let lives = 5;
let score = 0;
let currentAttempt = 1;

function preguntasAleatorias() {
  if (lives > 0 && randomQuestions.length > 0) {
    const randomIndex = Math.floor(Math.random() * randomQuestions.length);
    const randomQuestion = randomQuestions[randomIndex];
    const question = randomQuestion.question;
    const answers = randomQuestion.answers;
    const correctAnswer = randomQuestion.correct;

    let userAnswer = prompt(`${question}\n\n${answers.join("\n")}`);

    let attempts = 1;
    while (attempts < 3) {
      if (userAnswer === correctAnswer) {
        if (attempts === 1) {
          score += 10;
          alert(`¡Correcto! Ganaste 10 puntos.`);
        } else {
          score += 5;
          alert(`¡Correcto! Ganaste 5 puntos.`);
        }
        randomQuestions.splice(randomIndex, 1);
        break;
      } else {
        lives--;
        alert(`Incorrecto. Te quedan ${lives} vidas.`);
        attempts++;
        if (lives === 0) {
          break;
        }
        userAnswer = prompt(`${question}\n\n${answers.join("\n")}`);
      }
    }

    if (attempts === 3) {
      alert(
        `Incorrecto. Te quedan ${lives} vidas \n y saltamos a la siguiente pregunta`
      );
      randomQuestions.splice(randomIndex, 1);
      currentQuestion++;
    }

    preguntasAleatorias();
  } else if (lives === 0) {
    alert(`Has perdido todas tus vidas. Tu puntuación es ${score}.`);
  } else {
    alert(`Has respondido todas las preguntas. Tu puntuación es ${score}.`);
  }
}

const randomQuestions = quizQuestions.slice().sort(() => Math.random() - 0.5);

preguntasAleatorias();

/* vidas(); */
preguntas();
